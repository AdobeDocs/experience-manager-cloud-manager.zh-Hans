...
>[!NOTE]
>
>此处提供的代码示例仅用于说明目的。 要了解其概念和质量规则，请参阅[SonarQube的概念文档](https://docs.sonarsource.com/sonarqube-server)。
...
### 请勿使用受外部控制的格式字符串 {#do-not-use-format-strings-which-may-be-externally-controlled}
...
使用来自外部源的格式字符串（如请求参数或用户生成的内容）可能会使应用程序遭受拒绝服务攻击。 在某些情况下，虽然格式字符串受到外部控制，但仅允许来自受信任源。
...
从 AEM 应用程序内部执行 HTTP 请求时，配置适当的超时以避免不必要的线程消耗至关重要。 不幸的是，Java™ 的默认 HTTP 客户端 `java.net.HttpUrlConnection` 和广泛使用的 Apache HTTP 组件客户端都没有默认超时。 因此，必须明确配置超时。 作为最佳实践，这些超时不超过60秒。
...
通常，一个异常应只记录一次。 多次记录异常可能会引起混乱，因为不清楚异常发生了多少次 导致出现此问题的最常见模式是记录并抛出捕获的异常。
...
另一个要避免的常见模式是记录一条消息，然后立即引发异常。 此问题表示异常消息在日志文件中重复。
...
通常，应使用 INFO 日志级别来划分重要操作，默认情况下，AEM 配置为在 INFO 级别或更高级别进行记录。 GET 和 HEAD 方法只能为只读操作，因此，不会构成重要操作。 在INFO级别进行记录以响应GET或HEAD请求会生成大量日志噪音，导致更难以识别日志文件中的有用信息。 在处理GET或HEAD请求时，如果出现错误，则在WARN或ERROR级别进行记录。 有关更深入的故障排除信息，请记录DEBUG或TRACE级别。
...
作为最佳实践，日志消息应提供有关应用程序中发生异常的位置的上下文信息。 虽然使用栈栈跟踪来确定上下文，但通常日志消息更易于阅读和理解。 因此，在记录异常时，将异常消息用作日志消息是一种不好的做法。 异常消息详细介绍了错误情况。 相反，日志消息会告知读者当出现异常时应用程序正在做什么。 仍会记录异常消息。 通过指定您自己的消息，可使日志更易于理解。
...
### 在catch块中记录是在WARN或ERROR级别 {#logging-in-catch-blocks-should-be-at-the-warn-or-error-level}
...
上下文对于理解日志消息是至关重要的。 使用`Exception.printStackTrace()`会导致仅将栈栈跟踪输出到标准错误流，从而忽略所有上下文。 此外，在像AEM这样的多线程应用程序中，并行栈栈跟踪输出会导致问题。 应仅通过记录框架来记录异常。
...
AEM 中的记录应始终通过记录框架 SLF4J 完成。 直接输出到标准输出或标准错误流将丢失记录框架提供的结构和上下文信息，并且有时会导致性能问题。
...
以`/libs`和`/apps`开头的路径未硬编码。 这些路径通常相对于`Sling`搜索路径进行存储，该路径默认为`/libs,/apps`。 使用绝对路径会引入细微的缺陷，这些缺陷仅在项目生命周期的后期出现。
...
请勿将Sling调度程序用于需要保证执行的任务。 Sling计划作业可保证执行，并且更适合集群和非集群环境。

要详细了解如何在集群环境中处理Sling作业，请参阅[Apache Sling事件和作业处理文档](https://sling.apache.org/documentation/bundles/apache-sling-eventing-and-job-handling.html)。

### 不要使用AEM已弃用的API {#sonarqube-aem-deprecated}
...
通常，使用标准Java弃用这些API™ *@Deprecated*&#x200B;注释将其标识为`squid:CallToDeprecatedMethod`。

但在某些情况下，API会在AEM上下文中被弃用，但在其他上下文中并未被弃用。 此规则标识第二个类。
...
>[!NOTE]
>
>OakPAL 是一个框架，它使用独立的 Oak 存储库来验证内容包。 AEM合作伙伴、2019年“AEM北美摇滚明星”奖得主开发了该产品。

### 不要实施或扩展用@ProviderType注释的产品API {#product-apis-annotated-with-providertype-should-not-be-implemented-or-extended-by-customers}
...
### 请勿在客户包中的`/libs`下创建或编辑节点 {#oakpal-customer-package}
...
客户应将 AEM 内容存储库中的 `/libs` 内容树视为只读，这是一个长期存在的最佳实践。 修改 `/libs` 下的节点和属性会给主要和次要更新带来重大风险。 Adobe仅通过官方渠道对`/libs`进行编辑。
...
与[包不应包含重复的OSGi配置规则](#oakpal-package-osgi)类似，这个问题是多个单独的内容包写入同一节点路径的复杂项目中的常见问题。 虽然可使用内容包依赖项来确保获得一致的结果，但最好是完全避免重叠。

### 默认创作模式不是经典UI {#oakpal-default-authoring}
...
### 带有对话框的组件需要触控UI对话框 {#oakpal-components-dialogs}
...
### 启用代理的客户端库中包含的资源位于名为resources的文件夹中 {#oakpal-resources-proxy}
...
AEM客户端库可以包含静态资源，如图像和字体。 如[“使用客户端库”文档](https://experienceleague.adobe.com/zh-hans/docs/experience-manager-65/content/implementing/developing/introduction/clientlibs#using-preprocessors)中所述，在使用代理客户端库时，这些静态资源必须包含在名为 `resources` 的子文件夹中，才能在发布实例上有效引用这些资源。
...
### 禁止自定义某些标准索引定义 {#oakpal-customizing-ootb-index}
...
AEM Cloud Service禁止对以下标准索引进行未经授权的修改：
...
* [Dispatcher配置已弃用的属性](https://github.com/adobe/aem-dispatcher-optimizer-tool/blob/main/docs/Rules.md#dot---parsing-violation---dispatcher-configuration-deprecated-property)
...
* [Dispatcher发布场缓存已启用`serveStaleOnError`](https://github.com/adobe/aem-dispatcher-optimizer-tool/blob/main/docs/Rules.md#dot---the-dispatcher-publish-farm-cache-should-have-servestaleonerror-enabled)

* [Dispatcher发布场过滤器应包含来自AEM原型6.x.x版本的默认拒绝规则](https://github.com/adobe/aem-dispatcher-optimizer-tool/blob/main/docs/Rules.md#dot---the-dispatcher-publish-farm-filters-should-contain-the-default-deny-rules-from-the-6xx-version-of-the-aem-archetype)

* [Dispatcher发布场缓存`statfileslevel`属性为>= 2](https://github.com/adobe/aem-dispatcher-optimizer-tool/blob/main/docs/Rules.md#dot---the-dispatcher-publish-farm-cache-statfileslevel-property-should-be--2)

* [Dispatcher发布场`gracePeriod`属性为>= 2](https://github.com/adobe/aem-dispatcher-optimizer-tool/blob/main/docs/Rules.md#dot---the-dispatcher-publish-farm-graceperiod-property-should-be--2)

* [每个Dispatcher场都有一个唯一的名称](https://github.com/adobe/aem-dispatcher-optimizer-tool/blob/main/docs/Rules.md#dot---each-dispatcher-farm-should-have-a-unique-name)

* [Dispatcher发布场缓存已以方式配置其`ignoreUrlParams`规则](https://github.com/adobe/aem-dispatcher-optimizer-tool/blob/main/docs/Rules.md#dot---the-dispatcher-publish-farm-cache-should-have-its-ignoreurlparams-rules-configured-in-an-allow-list-manner)

* [Dispatcher发布场过滤器以方式指定允许的Sling选择器](https://github.com/adobe/aem-dispatcher-optimizer-tool/blob/main/docs/Rules.md#dot---the-dispatcher-publish-farm-filters-should-specify-the-allowed-sling-selectors-in-an-allow-list-manner)

* [Dispatcher发布场过滤器以方式指定允许的Sling后缀模式](https://github.com/adobe/aem-dispatcher-optimizer-tool/blob/main/docs/Rules.md#dot---the-dispatcher-publish-farm-filters-should-specify-the-allowed-sling-suffix-patterns-in-an-allow-list-manner)
