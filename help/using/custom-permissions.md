...
了解如何使用自定义权限创建新的自定义权限配置文件，以及可配置的权限来限制对Cloud Manager用户的项目、管道和环境的访问。
...
自定义权限允许用户创建新的自定义权限配置文件，可配置权限以限制Cloud Manager用户访问程序、管道和环境。
...
此部分详细介绍了这三个步骤。 在创建自己的自定义权限时，请参阅[条款](#terms)和[可配置权限](#configurable-permissions)部分。

>[!NOTE]
>
>要创建新配置文件并管理Cloud Manager的权限，您必须在Admin Console中拥有产品管理员权限。
...
1. 搜索名称与模式`*-cloud-manager`匹配的实例，并选择它以管理用户和权限。
...
1. 在Admin Console中，单击[新产品配置文件](#create)的名称。
...
Cloud Manager 中组织级权限的一个示例是&#x200B;**存储库信息访问权限**。 此权限允许用户生成用户名、密码和存储库 URL，以访问客户项目并为其做出贡献。 虽然组织中的所有存储库的用户名和密码相同，但每个项目都有一个唯一的存储库URL。
...
* 在Cloud Manager中创建的项目、环境和管道等资源需要两分钟时间才能在Admin Console中显示以进行权限配置。
* 如果自定义权限服务无法响应，则预定义用户档案仍然可用，并且预定义用户档案中的用户仍然具有适当访问权限。
...
### 引入自定义配置文件后，预定义权限配置文件会发生什么情况？

默认产品配置文件和 Cloud Manager 角色仍将照常发挥作用。

### 是否能编辑预定义的权限配置文件？

否，默认配置文件是不可编辑的。 您无法从默认权限配置文件中添加或删除权限。 您只能在预定义的配置文件中添加或删除用户。
...
是。 用户可以包含在多个配置文件中，包括预定义和自定义权限配置文件。 在将一个用户分配给多个配置文件时，所有分配的权限配置文件中的组合权限可供该用户使用。
