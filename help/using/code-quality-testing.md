...
有100多条规则结合了通用Java规则和特定于AEM的规则。 某些特定于AEM的规则是根据AEM Engineering的最佳实践创建的，这些规则称作[自定义代码质量规则](/help/using/custom-code-quality-rules.md)。

>[!TIP]
>
>您可以使用此链接](/help/assets/CodeQuality-rules-latest-AMS.xlsx)下载规则[的完整列表。
...
>[!NOTE]
>
>要了解有关[!UICONTROL Cloud Manager]执行的自定义代码质量规则的更多信息，请参阅[自定义代码质量规则](custom-code-quality-rules.md)。
...
您可以在[管道配置](/help/using/production-pipelines.md)的&#x200B;**测试**&#x200B;选项卡上从一到三个集合中进行选择。 流量分配基于所选页面集数。 也就是说，如果选定所有三个页面集，则每个页面集中都会获得总页面查看次数的33%。 如果选定两个页面集，则每个页面集的查看次数占总页面查看次数的 50%。如果选定一个页面集，则该页面集的查看次数占总页面查看次数的 100%。
...
Cloud Manager 通过在暂存发布服务器上以未经身份验证的用户身份（默认情况下）请求页面，来对 AEM Sites 项目执行性能测试，测试期间为 30 分钟。它测量每个页面的虚拟用户生成的量度（响应时间、错误率、每分钟查看次数等等），以及所有实例的各种系统级量度（CPU、内存、联网数据）。
...
Cloud Manager使用CSE设置的用户名和密码在author实例上创建文件夹。 之后，使用开源库将资产上传到该文件夹中。Assets测试步骤运行的测试是使用[开源库](https://github.com/adobe/toughday2)编写的。 每个资产的处理时间和各种系统级量度都会在30分钟的测试持续时间内测量。 此功能可以上传图像和 PDF 文档。
