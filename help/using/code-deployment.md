...
在部署的每个步骤中都会发生多个操作，本节将对此进行介绍。 有关如何部署代码本身的技术详细信息，请参阅[部署进程详细信息](#deployment-process)。

### 暂存部署步骤 {#stage-deployment}

**暂存部署**&#x200B;步骤包含以下操作：

* **验证** – 此步骤可确保将管道配置为使用当前可用的资源。 例如，存在配置的分支并且环境可用。
* **构建和单元测试**：此步骤运行容器化的构建过程。 有关详细信息，请参阅[构建环境](/help/getting-started/build-environment.md)。
...
Cloud Manager将生成过程生成的所有`target/*.zip`文件上传到存储位置。 在管道的部署阶段，将从该位置检索这些工件。

当Cloud Manager部署到非生产拓扑时，目标是尽可能高效地完成部署，因此工件将同时部署到所有节点，如下所示：

1. Cloud Manager 确定每个工件是 AEM 还是 Dispatcher 包。
1. Cloud Manager 会从负载平衡器中移除所有 Dispatcher，以在部署期间隔离环境。

   * 除非另有配置，否则您可以跳过开发和暂存部署中的负载均衡器更改。 即，对于开发环境，在两个非生产管道中分离和附加步骤；对于暂存环境，分离和附加步骤。
...
1. 每个 AEM 工件均通过包管理器 API 部署到每个 AEM 实例，其中包依赖关系将确定部署顺序。
   ...
   * 了解有关如何使用包安装新功能、在实例之间传输内容以及备份存储库内容的更多信息。 请参阅[包管理器](https://experienceleague.adobe.com/zh-hans/docs/experience-manager-cloud-service/content/implementing/developer-tools/package-manager)。
...
1. Dispatcher 工件将会部署到每个 Dispatcher，如下所示：

   1. 当前配置已备份并复制到临时位置。
   1. 已删除所有配置（不可变文件除外）。 有关更多详细信息，请参阅 [Dispatcher 配置](/help/getting-started/dispatcher-configurations.md)。 此方法会清除目录，以确保不会保留孤立文件。
   1. 工件将提取到 `httpd` 目录。 不会覆盖不可变文件。 在部署时，您对 Git 存储库中不可变文件所做的任何更改都会被忽略。 这些文件对于AMS Dispatcher框架至关重要，无法更改。
   1. Apache 执行配置测试。 如果未发现任何错误，则将重新加载服务。 如果发现错误，则从备份中恢复配置，重新加载服务，并将错误报告回Cloud Manager。
   1. 管道配置中指定的每个路径都将失效或从 Dispatcher 缓存中进行刷新。

   >[!NOTE]
   >
   >Cloud Manager 要求 Dispatcher 工件包含完整文件集。 所有 Dispatcher 配置文件都必须位于 Git 存储库中。 缺少文件或文件夹会导致部署失败。

1. 在将所有 AEM 和 Dispatcher 包成功部署到所有节点后，会将 Dispatcher 重新添加到负载平衡器，同时完成部署。

   >[!NOTE]
   >
   >您可以跳过开发和暂存部署中的负载均衡器更改。 也就是说，对于开发环境，在非生产管道中分离和附加步骤，对于暂存环境，在生产管道中进行分离和附加。

### 部署到生产阶段 {#deployment-production-phase}

部署到生产拓扑的过程略有不同，以尽量减小对 AEM 网站访客产生的影响。

生产部署遵循与上述相同的步骤，但它采用的是滚动方式：
...
在紧急情况下，Adobe Managed Services客户需要立即将代码更改部署到暂存环境和生产环境。 此功能允许他们绕过整个Cloud Manager测试周期。

为了解决这些情况，可以在紧急模式下执行Cloud Manager生产管道。 在使用此模式时，不执行安全性测试和性能测试步骤。 所有其他步骤（包括任何已配置的审批步骤）都会像在正常管道执行模式中那样执行。

>[!NOTE]
>
>紧急管道执行模式功能是逐个程序激活的。 客户成功工程师执行激活。
...
在极少数情况下，生产部署步骤可能会由于临时原因而失败。 在这些情况下，只要生产部署步骤已完成，您就可以重新执行它，无论它是成功、遭到取消还是不成功。 使用由以下三个步骤组成的相同管道支持重新执行：

1. **验证步骤** - 在正常管道执行期间进行的相同验证。
1. **构建步骤** — 在重新执行的上下文中，构建步骤将复制工件，而不会执行新的构建过程。
...
向此端点提交 `PUT`请求会导致出现 `201` 响应（如果成功）。 响应主体表示的是新的执行。 该功能类似于通过 API 开始常规执行。
