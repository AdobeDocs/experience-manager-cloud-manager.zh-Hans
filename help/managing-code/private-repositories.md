...
>[!NOTE]
>
>此功能为公共 GitHub 所独有。 不支持自托管的 GitHub。
...
1. 提供存储库的以下详细信息：
   ...
   * **存储库名称** — 描述性名称。
   * **存储库URL** — 存储库的URL，必须以`.git`结尾。
   * **描述**（可选） — 根据需要对存储库进行更长的描述。
...
Cloud Manager现在已配置您的GitHub存储库，但仍需要授权才能访问它。 要授予访问权限，您需要安装 Adobe GitHub 应用程序并验证您是否拥有指定的存储库。
...
1. Cloud Manager 使用 GitHub 应用程序与您的存储库安全地交互。
   ...
   GitHub组织的所有者必须安装位于`https://github.com/apps/cloud-manager-for-aem`的应用程序并提供对存储库的访问权限。 有关详细信息，请参阅 GitHub 的文档。
...
1. 在GitHub存储库的默认分支中创建一个名为`.well-known/adobe/cloud-manager-challenge`的新文件，将机密文件内容粘贴到该文件中并保存。
...
* 如果从您的GitHub组织中删除了Adobe GitHub应用程序，则此操作将删除适用于所有存储库的拉取请求验证功能。
