...
1. 在每个相应的字段中，提供有关存储库的以下详细信息：
   ...
   | **存储库 URL** | 必需。 存储库的 URL。<br><br>如果您使用的是GitHub托管的存储库，则路径必须以`.git`结尾。<br>例如，*`https://github.com/org-name/repo-name.git`* （URL路径仅供说明使用）。<br><br>如果您使用的是外部存储库，则必须使用以下URL路径格式：<br>`https://git-vendor-name.com/org-name/repo-name.git`<br>或<br>`https://self-hosted-domain/org-name/repo-name.git`<br>并且与您的Git供应商匹配。 |
...
1. 选择&#x200B;**保存**&#x200B;以添加存储库。
   ...
   要验证外部存储库的所有权，请提供访问令牌。

1. 要验证外部存储库的所有权以便您能够访问它，请在&#x200B;**私有存储库所有权验证**&#x200B;对话框中提供一个访问令牌，然后单击&#x200B;**验证**。
...
1. 要发送Cloud Manager所需的事件，请配置webhook。 使用下表确定Git提供商的正确事件。
...
| 必需的webhook事件 |
| --- |
| 这些webhook事件允许Cloud Manager在推送代码或提交合并请求时触发管道。 它们还跟踪与拉取请求验证相关的注释（通过注释事件）。<br>确保将webhook设置为在以下必需的webhook事件中触发<ul><li>推送事件<li>合并请求事件<li>注释事件</li></li></li></ul></ul></ul> |
...
创建检查后，它类似于以下屏幕快照。 与`GitHub.com`的主要区别在于`GitHub.com`使用检查运行，而GitHub Enterprise（使用个人访问令牌）生成提交状态：
...
GitLab交互仅依赖注释。 验证开始时，将添加注释。 验证完成（无论成功还是失败）后，将移除初始注释，并替换为包含验证结果或错误详细信息的新注释。
...
![代码质量验证完成后](/help/managing-code/assets/repository-webhook-gitlab2.png)
