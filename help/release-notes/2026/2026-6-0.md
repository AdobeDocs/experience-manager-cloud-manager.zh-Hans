...
>[!IMPORTANT]
>
>Beta版本包含问题，并且不提供担保。 Adobe没有义务维护、更正、更新、更改、修改或以其他方式支持（通过Adobe支持服务或其他方式）Beta版。 客户使用测试版需自行承担风险。 请勿依赖测试版版本的正确功能或性能，或者依赖任何随附的文档或材料。 Beta版中的功能和API如有更改，恕不另行通知。 任何使用测试版的风险完全由客户自行承担。
...
### 通过模块缓存提高了生成性能 {#quick-build-cm-pipelines}

新的构建模型使用模块级缓存提高构建性能，只编译已更改的模块（而不是整个存储库）。 它适用于代码质量和全栈管道。

![编辑非生产管道对话框，其中显示了“完全生成”和“智能生成”这两个生成策略选项](/help/release-notes/assets/non-production-pipeline-edit.png)
*编辑非生产管道对话框，其中显示两个生成策略选项，即“完全生成”和“智能生成”。*
